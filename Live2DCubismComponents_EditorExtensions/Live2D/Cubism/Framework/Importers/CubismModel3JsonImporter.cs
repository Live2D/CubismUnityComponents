...
namespace Live2D.Cubism.Framework.EditorExtensions.Importers
{
    /// <summary>
    ...
    public sealed class CubismModel3JsonImporter : CubismImporterBase
    {
        ...
        private struct Set<T> where T : Component
        {
            ...
            public static Set<T> From(T[] a, T[] b)
            {
                return new Set<T>
                {
                    ANotB = a.Where(i => b.First(j => j.name == i.name) == null).ToArray(),
                    BNotA = b.Where(i => a.First(j => j.name == i.name) == null).ToArray(),
                    AAndB = a
                        .Where(i => b.First(j => j.name == i.name) != null)
